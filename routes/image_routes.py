...
def get_reviews():
    '''
    Endpoint returning all reviews.
    ---
    tags:
      - review-rating-service
    parameters:
                  ...
                  update:
                    ...
                    properties:
                      ...
                      method:
                        ...
                        example: "PUT"
    '''
    dish_id_filter = request.args.get('dish_id')
    rating_filter = request.args.get('rating')
    updated_at_filter = request.args.get('updated_at')

    query = db.session.query(Review)
    if dish_id_filter:
        query = query.filter(Review.dish_id == dish_id_filter)
    if rating_filter:
        query = query.filter(Review.rating >= rating_filter)  
    if updated_at_filter:
        query = query.filter(Review.updated_at >= updated_at_filter)
